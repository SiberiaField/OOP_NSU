...
void CircularBuffer::rotate(int new_begin){
	if(0 <= new_begin && new_begin < size){
		new_begin = normalize(beg + new_begin);
		value_type* copy = make_copy();
...
//Выделение память при пустом buffer
//Добавить обработку неверного new_capacity
void CircularBuffer::set_capacity(int new_capacity){
	if(new_capacity < 1){
		throw std::invalid_argument("new_capacity must be > 1");
	}
	if(buffer == nullptr){
	...
	if(new_capacity != capacity){
		...
		if(!this->empty()){
			if(new_capacity > capacity){
				...
				int i = beg, j = 0;
				for(; j < new_capacity && i != normalize(end + 1); i++, j++){
					i = normalize(i);
